...
function deleteToDo(event){
    ...
    const tr = btn.parentNode; // 그리고 그 부모 태그(li)를 할당.
    toDoList.removeChild(tr); // 해당 태그를 지움.
...
function paintToDo(text){
    const tr = document.createElement("tr"); // li 태그를 만들어서 변수에 할당
    ...
    tr.appendChild(span); // li 태그의 자식 태그로 span 태그 삽입
    tr.appendChild(delBtn); // li 태그의 자식 태그로 버튼을 삽입
    tr.id = newId; // li태그의 id 속성을 object의 id와 같게 함.
    toDoList.appendChild(tr); // 위에서 설정한 li 태그들을 최종적으로 toDoList에 삽입.
    const toDoObj = {
        text : text,
        id : newId
    };
